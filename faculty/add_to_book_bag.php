...
$facultyId = $_SESSION['Faculty_Id']; // Assuming the student ID is stored in the session

// Check how many books the student currently has borrowed
$query = "SELECT COUNT(*) AS borrowed_count FROM borrow WHERE faculty_id = ? AND (status = 'pending' OR status = 'ready_to_claim' OR status = 'borrowed' OR status = 'lost')";
...
// Check if the book already exists in the session to prevenst duplicates based on title and author
$bookExists = false;
