...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($stmtBorrow === false) {
            echo json_encode(['success' => false, 'message' => 'Error preparing the borrow update statement. ' . $conn->error]);
            exit;
        }

        // Bind parameters for the borrow update based on user type
        if ($bindType === 'i') {
            $stmtBorrow->bind_param('dsiiss', $fineAmount, $damageDescription, $adminId, $userId, $bookId, $category);
        } else {
            $stmtBorrow->bind_param('dsisss', $fineAmount, $damageDescription, $adminId, $userId, $bookId, $category);
        }
        ...
        if ($stmtAccession === false) {
            echo json_encode(['success' => false, 'message' => 'Error preparing the accession update statement. ' . $conn->error]);
            $stmtBorrow->close();
            $conn->close();
            exit;
        }
        ...
        $stmtAccession->bind_param('ssss', $damageDescription, $accessionNo, $userId, $bookId);
